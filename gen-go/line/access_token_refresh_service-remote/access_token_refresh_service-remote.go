...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  RefreshAccessTokenResponse refresh(RefreshAccessTokenRequest request)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := line.NewAccessTokenRefreshServiceClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "refresh":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "Refresh requires 1 args")
      flag.Usage()
    }
    arg942 := flag.Arg(1)
    ...
    argvalue0 := line.NewRefreshAccessTokenRequest()
    ...
    value0 := argvalue0
    fmt.Print(client.Refresh(context.Background(), value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
